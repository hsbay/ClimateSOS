...
def _bound_pathway(state: BoundState) -> BoundPathway:
    return BoundPathway(
...
def test_every_implemented_bound_state_is_independently_representable() -> None:
    ...
    represented = tuple(_bound_pathway(state) for state in BoundState)
...
def test_no_ack_and_unbound_are_distinct_immutable_states() -> None:
    no_ack = _bound_pathway(BoundState.NO_ACK)
    unbound = _bound_pathway(BoundState.UNBOUND)
...
def test_binding_handler_is_concrete_work_performing_component() -> None:
    bind_signature = inspect.signature(BindingHandler.bind)
    bind_hints = get_type_hints(BindingHandler.bind)

    assert inspect.isclass(BindingHandler)
...
def test_removed_architecture_drift_is_not_publicly_exported() -> None:
    removed = {
    ...
    assert removed.isdisjoint(pathway_evaluation.__all__)
    assert all(not hasattr(pathway_evaluation, name) for name in removed)
