...
After the Integrated Charter Evaluation, the data available in or referenced by
`PathwayEngineResult` and `IntegratedCharterResult` are ready for processing
to determine their overall evaluated effect on the broader transition system.

The `ProductPathway` produces one or more declared outputs, but those outputs
do not in themselves establish a contribution to the accelerated net-zero transition.
ClimateSOS therefore distinguishes the pathway's outputs from their evaluated
effects on the broader transition system.

The `NetOverallSystemContributionEvaluator` evaluates those effects after
pathway comparison, queue and fabric evaluation, documentation evaluation, and
the Integrated Charter Evaluation have completed. It determines how the
evaluated pathway compares with and interacts with the authoritative
`TransitionPathway` and records the contribution substantiated by the completed
evaluation record.
...
The PathwayEngineResult will contain one or more outputs generated by the 
ProductPathway. Those outputs may include physical goods, materials, energy
or resource flows, infrastructure or operating assets, services, system 
functions, financial or institutional mechanisms, information or 
coordination capabilities, physical or environmental effects, or other 
results produced by the represented pathway.

Examples may include:

* clean electricity;
* clean or lower-carbon fuel;
* industrial heat;
* hydrogen or another industrial feedstock;
* stored carbon;
* methane abatement;
* grid flexibility or adequacy support;
* waste reduction or material recovery;
* industrial products or materials;
* transport services or fuels;
* transmission, storage, or other infrastructure service;
* transition-finance capability;
* workforce-transition capability; or
* verification, evidence, or coordination services.

Product outputs are represented on or traceable through the `ProductPathway`
and its associated intake material.

The `NetOverallSystemContributionEvaluator` does not change a declared output
into a different output in order to improve its evaluation. It evaluates what
the represented output does within the transition system.

An output may be technically useful, commercially valuable, locally beneficial,
or operationally feasible without making a material net overall system
contribution.
