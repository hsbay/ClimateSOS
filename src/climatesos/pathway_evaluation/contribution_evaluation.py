"""Net overall system contribution evaluation and invariant enforcement."""
...
class NetOverallSystemContributionEvaluator:
    """Evaluate contribution domains and construct the aggregate contribution result."""
    ...
    def evaluate(
        ...
        """Evaluate each contribution domain once and preserve domain order."""

        self._validate_inputs(pathway_engine_result, integrated_charter_result)

        fossil_findings = _require_tuple_of(
            self.fossil_contribution_function(
                pathway_engine_result,
                integrated_charter_result,
            ),
            ContributionFinding,
            "Fossil-displacement contribution findings",
        )
        reliability_findings = _require_tuple_of(
            self.reliability_delivery_timing_function(
                pathway_engine_result,
                integrated_charter_result,
            ),
            ContributionFinding,
            "Reliability, delivery, and timing contribution findings",
        )
        enabling_findings = _require_tuple_of(
            self.enabling_demand_burden_function(
                pathway_engine_result,
                integrated_charter_result,
            ),
            ContributionFinding,
            "Enabling-demand and burden contribution findings",
        )
        emissions_findings = _require_tuple_of(
            self.emissions_cdr_biosphere_function(
                pathway_engine_result,
                integrated_charter_result,
            ),
            ContributionFinding,
            "Emissions, CDR, and biosphere contribution findings",
        )
    ...
    def _validate_inputs(
        ...
        if integrated_charter_result.pathway_engine_result is not pathway_engine_result:
            raise ContributionEvaluationInvariantError(
        ...
        if (
            ...
            or integrated_charter_result.evaluation_run_id != pathway.evaluation_run_id
        ):
            raise ContributionEvaluationInvariantError(
    ...
    def _validate_finding(
        ...
        if any(
            ...
            raise ContributionEvaluationInvariantError(
                "Supporting documentation findings must come from PathwayEngineResult"
            )
